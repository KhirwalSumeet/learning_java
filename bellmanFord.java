...
class adjacencyList implements GraphInterface {
    ...
    public LinkedList<Edge> getAllEdges() {
        ...
        for (int i=0; i < vertexCount; i++) {
            ...
            edges = (LinkedList<Edge>) data.get(i).clone();
            while (edges.peek() != null) {
                allEdges.add(edges.poll());
            }
        }
        return (LinkedList<Edge>) allEdges.clone();
    }
}
        ...
        for (int i=0; i < maxSize; i++) {
            for (int j=0; j < maxSize; j++) {
                if (matrix[i][j] != Integer.MAX_VALUE)
                    allEdges.add(new Edge(matrix[i][j], j, i));
            }
        }
        return allEdges;
    }
...
class Graph implements GraphInterface {
    ...
    public LinkedList<Edge> getAllEdges() {
        return type == "Matrix"? am.getAllEdges() : al.getAllEdges();
    }
...
class GraphDemo {
    ...
    public void bellmanFord(Graph gr) {
        int vertexCount = gr.vertexCount;
        int[] distance = new int[vertexCount];
        Arrays.fill(distance, Integer.MAX_VALUE);
        distance[0] = 0;
        LinkedList<Edge> edges = gr.getAllEdges();
        for (int i = 0; i < vertexCount; i++) {
            Iterator x = edges.listIterator();
            while (x.hasNext()) {
                Edge e = (Edge) x.next();
                if (distance[e.src] < Integer.MAX_VALUE) {
                    if (distance[e.dest] > distance[e.src] + e.weight) {
                        distance[e.dest] = distance[e.src] + e.weight;
                    }
                }
            }
        }
        boolean hasNegativeWt = checkNegativeWeights(distance, edges, vertexCount);
        System.out.println("Do the graph have negative cycles ? " + hasNegativeWt);
    }

    boolean checkNegativeWeights(int[] distance, LinkedList<Edge> edges, int vertexCount) {
